...
DIM i&
i& = Robohash("SpriggsySpriggs", 1)

IF i& <> 0 AND i& <> -1 THEN
    SCREEN i&
END IF

FUNCTION Robohash& (stringhash AS STRING, setnumber AS INTEGER)
    DIM URL AS STRING
    DIM getpath AS LONG
    ...
    location = "/" + stringhash + ".png?set=set" + _TRIM$(STR$(setnumber)) + CHR$(0)
    ...
    Robohash = _LOADIMAGE(tempfile, 32)
    KILL tempfile
END FUNCTION
